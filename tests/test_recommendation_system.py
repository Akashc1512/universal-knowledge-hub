...
class TestContentBasedFiltering:
    ...
    async def test_get_recommendations(self, mock_graph_client):
        ...
        # Mock recommendation data
        mock_recommendations = [
            {"document_id": "doc1", "title": "Tech Doc", "topic": "technology", "topic_preference": 0.8},
            {"document_id": "doc2", "title": "Science Doc", "topic": "science", "topic_preference": 0.6},
        ]
...
class TestHybridRecommendationEngine:
    ...
    async def test_get_recommendations(self, hybrid_engine, mock_graph_client):
        ...
        with patch.object(hybrid_engine.collaborative_filtering, 'get_recommendations', 
                        return_value=collaborative_recs), \
             patch.object(hybrid_engine.content_based_filtering, 'get_recommendations', 
                        return_value=content_recs), \
             patch.object(hybrid_engine.semantic_filtering, 'get_recommendations', 
                        return_value=semantic_recs):
            ...
            assert result.algorithms_used == ["collaborative_filtering", "content_based_filtering", "semantic_filtering"]
