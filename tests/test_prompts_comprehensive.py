...
try:
    ...
    class QueryProcessingPrompts:
        def get_classification_prompt(self, query): return f"Classify: {query}"
        def get_entity_extraction_prompt(self, query): return f"Extract entities: {query}"
        def get_complexity_prompt(self, query): return f"Assess complexity: {query}"
        def get_intent_prompt(self, query): return f"Recognize intent: {query}"
        def get_refinement_prompt(self, query, context): return f"Refine: {query} with {context}"
    
    class RetrievalPrompts:
        def get_semantic_search_prompt(self, query): return f"Semantic search: {query}"
        def get_keyword_search_prompt(self, query): return f"Keyword search: {query}"
        def get_hybrid_search_prompt(self, query): return f"Hybrid search: {query}"
        def get_reranking_prompt(self, query, documents): return f"Rerank: {query}"
    
    class SynthesisPrompts:
        def get_answer_generation_prompt(self, query, documents): return f"Generate answer: {query}"
        def get_aggregation_prompt(self, documents): return f"Aggregate: {len(documents)} docs"
        def get_summarization_prompt(self, content): return f"Summarize: {content[:50]}"
        def get_explanation_prompt(self, concept, context): return f"Explain: {concept}"
    
    class FactCheckPrompts:
        def get_verification_prompt(self, claim, sources): return f"Verify: {claim}"
        def get_decomposition_prompt(self, claim): return f"Decompose: {claim}"
        def get_evidence_evaluation_prompt(self, claim, evidence): return f"Evaluate: {claim}"
        def get_contradiction_detection_prompt(self, sources): return f"Detect contradictions"
    
    class CitationPrompts:
        def get_citation_generation_prompt(self, answer, sources): return f"Generate citations: {answer[:50]}"
        def get_relevance_scoring_prompt(self, query, sources): return f"Score relevance: {query}"
        def get_formatting_prompt(self, citations, format_style): return f"Format: {format_style}"
...
class TestFactCheckPrompts(unittest.TestCase):
    ...
    def test_claim_verification_prompt(self):
        ...
        sources = [
            ...
            {"content": "Classical computers are still faster for most tasks", "source": "textbook"}
        ]
...
class TestCitationPrompts(unittest.TestCase):
    ...
    def test_citation_generation_prompt(self):
        ...
        sources = [
            {"title": "Quantum Computing Basics", "url": "https://example.com/quantum", "content": "Quantum computing uses..."},
            {"title": "Advanced Quantum Algorithms", "url": "https://example.com/algorithms", "content": "Shor's algorithm can..."}
        ]
...
class TestPromptValidation(unittest.TestCase):
    ...
    def test_prompt_clarity_validation(self):
        ...
        bad_prompts = [
            ...
            "   ",
            "prompt",
            "this is not a proper prompt structure"
        ]
