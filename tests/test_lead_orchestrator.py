...
async def test_token_budgeting_enforcement():
    ...
    context = QueryContext(
        query="What is the capital of France?", token_budget=500  # Small budget to trigger limits
    )
    plan = {
...
async def test_per_agent_budget_calculation():
    orchestrator = LeadOrchestrator()
    context = QueryContext(query="Test query", token_budget=1000)
    plan = {
        "required_agents": [
            AgentType.RETRIEVAL,
    ...
    for agent_type in plan["required_agents"]:
        agent_budgets[agent_type] = orchestrator.token_controller.get_agent_budget(
            agent_type, context.token_budget
        )
...
async def test_token_usage_tracking():
    orchestrator = LeadOrchestrator()
    context = QueryContext(query="Test query for token tracking", token_budget=2000)
    plan = {
...
async def test_budget_exceeded_handling():
    orchestrator = LeadOrchestrator()
    context = QueryContext(query="Test query", token_budget=50)  # Very small budget
    plan = {
